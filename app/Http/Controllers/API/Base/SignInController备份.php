        ...
        foreach ($total_reward as $k => $v){
            if ($v->gift_bag_id){
                ...
                $is_receive = DB::table('user_gift_bag_log')
                    ...
                    ->first();
                $total_reward[$k]->gift_bag->gift_bag_received = $is_receive ? true : false;
            }
        }
...
//    累计签到奖励
    public function gift($user_id = 0 ,$total_signin_days = 0)
    {
        ...
        $data = DB::transaction(function () use ($user_id,$total_signin_days) {
            $setting = DB::table('signin_setting as s')
                ->leftJoin('tools as t','s.tools_id','t.id')
                ->where('s.day_type',1)
            ...
            if (!empty($setting)){
                if ($setting->reward_points || $setting->reward_diamonds){
                    $user_points1 = DB::table('user')->where('id', $user_id)
                        ->update([
                            'points' => DB::raw('points + '.$setting->reward_points),
                            'diamonds'  => DB::raw('diamonds + '.$setting->reward_diamonds),
                        ]);
                ...
                if ($setting->tools_id && $setting->tools_num) {
                    $backpack_id1 = DB::table('backpack')->insertGetId([
                        'user_id'=>$user_id,
                        'tools_id'=>$setting->tools_id,
                        'tools_num'=>$setting->tools_num
                    ]);
                ...
                if ($setting->gift_bag_id && $setting->gift_bag_num) {
                    $is_receive = DB::table('user_gift_bag_log')
...
//                  礼包中的元宝和铜钱
                    $gift_bag_points = DB::table('gift_bag')
                        ->where('id',$setting->gift_bag_id)
                        ->where('is_valid',1)
                        ->select('include_diamonds','include_points')
                        ->first();
                    if ($gift_bag_points->include_points || $gift_bag_points->include_diamonds){
                        $user_points2 = DB::table('user')->where('id', $user_id)
                            ->update([
                                'points' => DB::raw('points + '.$gift_bag_points->include_points*$setting->gift_bag_num),
                                'diamonds'  => DB::raw('diamonds + '.$gift_bag_points->include_diamonds*$setting->gift_bag_num),
                            ]);
                        ...
                        DB::table('user_account_log')
                            ->insert([
                                ...
                                'detail'=>date('n').'月累计签到'.$total_signin_days.'天，获得礼包，包含'.$gift_bag_points->include_diamonds.'元宝，'.$gift_bag_points->include_points.'铜钱',
                            ]);
                    }
                    //            礼包中的道具
                    $gift_bag_tools = DB::table('gift_bag_tools as g')
                        ->join('tools as t','g.tools_id','t.id')
                        ->where('g.gift_bag_id',$setting->gift_bag_id)
                    ...
                    foreach ($gift_bag_tools as $k => $v){
                        $backpack_id2 = DB::table('backpack')->insertGetId([
                            'user_id'=>$user_id,
                            'tools_id'=>$v->tool_id,
                            'tools_num'=>$v->number*$setting->gift_bag_num
                        ]);

                        DB::table('user_tools_log')->insert([
                            'user_id'=>$user_id,
                            'tools_id'=>$v->tool_id,
                            'tools_count'=>$v->number*$setting->gift_bag_num,
                            'behavior'=>405,
                            'gift_bag_id'=>$setting->gift_bag_id,
                        ]);
                    }
                    //            礼包领取记录
                    $log_id = DB::table('user_gift_bag_log')->insertGetId([
                        'user_id'=>$user_id,
                        'gift_bag_id'=>$setting->gift_bag_id,
                        'gift_bag_count'=>$setting->gift_bag_num,
                        'behavior'=>405,
                        'year'=>date('Y'),
                        'month'=>date('n'),
                        'day'=>$total_signin_days,
                    ]);
                }
                else{
                    return null;
                ...
                if (!empty($log_id)){
                    ...
                    $data['reward_points'] = $setting->reward_points;//累计签到奖励铜钱
                    $data['reward_diamonds'] = $setting->reward_diamonds;//累计签到奖励元宝
                    $data['gift_bag'] = [
                        'include_diamonds'=>$gift_bag_points->include_diamonds*$setting->gift_bag_num ?? 0,
                        'include_points'=>$gift_bag_points->include_points*$setting->gift_bag_num ?? 0,
